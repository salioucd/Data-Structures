...
def phonebook(names: list, numbers: list) -> dict:
    # dictionary of names in names list that are mapped to phone numbers in numbers list
    dictOfNamesMappedToPhoneNums = {}

    # while loop's counter
    whileLoopIteratorCountrVal = 0
    
    # run a while loop to loop on names list
    while whileLoopIteratorCountrVal != len(names):
        # current iteration name in names list
        currentIterationNameInNameList = names[whileLoopIteratorCountrVal]

        # current iteration number in numbers list
        currentIterationNumInNumsList = numbers[whileLoopIteratorCountrVal]
        # insert key as name and number as value in dictOfNamesMappedToPhoneNums dictionary
        dictOfNamesMappedToPhoneNums[currentIterationNameInNameList] = currentIterationNumInNumsList
        # increment while loop counter by 1
        whileLoopIteratorCountrVal = whileLoopIteratorCountrVal + 1

    # return the mapped dictionary
    return dictOfNamesMappedToPhoneNums
